# RTI Connext 7.x XCDR1 Cross-Vendor Captures

Wire-level byte captures of `Probe { octet a; double b; }` produced by
RTI Connext DDS 7.3.0 for XCDR1 (legacy CDR) interop reference.
...
**Observed:** RTI Connext 7.3.0 emits `@appendable` XCDR1 as plain
CDR v1 (encapsulation kind `PLAIN_CDR_LE`, 0x0001) with **no**
DHEADER, identical to both Fast DDS 3.1.2 and RTI Connext 6.1.0. The
body layout follows natural alignment per DDS-XTypes v1.3
Sec.7.4.3.4.1 Table 16.
...
Fast DDS 3.1.2 (`xcdr1_crossvendor/fastdds/`) and RTI Connext 6.1.0
(`xcdr1_crossvendor/rti6/`). Encapsulation kind, body bytes, padding
values, and IEEE 754 bit patterns match exactly across the three
reference vendors.

## Notes vs RTI Connext 6.1.0

- rtiddsgen bumped from 3.1.0 to 4.3.0; the new version parses
  `Probe.idl` without `-ppDisable` (the C-style include guards are
  now handled cleanly).
